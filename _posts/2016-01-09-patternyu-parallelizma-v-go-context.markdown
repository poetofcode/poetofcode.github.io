...
Метод `done` возвращает канал, который действует в качестве сигнала завершения для функций, запущенных от имени `context`а: когда канал закрыт, функции должны прекратить свою работу и завершиться. Метод `Err` возвращает ошибку, индицирующую почему Context был отменен. В статье [Конвейеры и аннулирование](http://blog.golang.org/pipelines) обсуждается идиома канала `Done` более детально.
